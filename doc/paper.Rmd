...
pull_ci(g_cf, digits = 3)
pull_ci(g_1ha, digits = 3)
rm(g_cf, g_1ha)
```
...
Plants were significantly larger in continuous forest (no overlap in 84% CI of model intercepts), and the disparity in plant size---which was most pronounced in the initial years of our surveys---diminished over time (Figure \@ref(fig:eda)a).
...
While it was 31% higher in continuous forest than 1-ha fragments (`r summary_stats$flwr_mean[2]` ± `r summary_stats$flwr_sd[2]` vs. `r summary_stats$flwr_mean[1]` ± `r summary_stats$flwr_sd[1]`, respectively), this difference was not statistically significant (84% CIs of model intercepts overlapped).
...
```{r include=FALSE}
repro_size <- model_data %>% filter(flwr == 1) %>% pull(size) %>% quantile(.1, na.rm = TRUE)
