...
In the current study we used the natural logarithm of the product of shoot number and height as a proxy for plant size.
Preliminary analysis showed that this was a better predictor of total leaf area than either shoot number or height alone (Table S2 <!--# update crossref manually -->).
...
```{r}
size_cap <- "Smooth effect of plant size in the previous census on a) survival, b) log(size), and c) flowering probability, modeled by cubic regression splines. Panel d) shows the smoothed density of different plant sizes in the two habitat types. 95% confidence intervals are shown and include uncertainty in the model intercept and uncertainty due to smoothness selection. The smooths for 1-ha fragments and continuous forest are fit in separate models. The model intercepts were added to the fitted values of the smooth and values are back-transformed to the response scale.  In b), a 1:1 line is shown in dashed red to indicate the line of no growth."
