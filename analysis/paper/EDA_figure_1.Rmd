...
There are plants with 0 shoots and 0 ht.
I will remove those, as having no above ground biomass is really not a size, but a different category.
...
Use INMET normals for Manaus from https://portal.inmet.gov.br/normais
...
ggsave(here("analysis", "figures", "manaus_normals.png"))
```
...
```
```{r}
xa %>% select(-spei_history, -L) %>% ungroup() %>%  summarize(spei = mean(spei)) %>% filter(year(yearmonth) == 2003) %>% arrange(spei)
```
...
surv_curve <-
  ...
  geom_step() +
  scale_x_yearmonth(limits = dates, breaks = census_dates, date_minor_breaks ="1 month", expand = expansion(mult = 0.04)) +
  scale_y_continuous("P(survived)") +
  theme_bw() +
  theme(
    axis.text.x = element_blank(),
...
size <-
  ...
  coord_cartesian(xlim = dates) + #limits here so points don't get removed
  theme_bw() +
  theme(
    axis.text.x = element_blank(),
    axis.title.x = element_blank(),
...
library(ggridges)
ggplot(demog_plotdf) +
  geom_density_ridges(aes(x = log(size), y = as.factor(year), fill = habitat, color = habitat), alpha = 0.5)
ggplot(demog_plotdf) +
  geom_violin(aes(y = log(size), x = as.factor(year), fill = habitat), alpha = 0.5, draw_quantiles = c(0.5))
...
```{r}
demog_plotdf %>% filter(year == 2003, shts >= 4) %>% group_by(habitat) %>% summarize(flwr = mean(flwr, na.rm = TRUE))
```
```{r}
demog_plotdf %>% 
  group_by(habitat, year) %>% 
  filter(!is.na(flwr)) %>% #removes both dead plants and plants with no data.
  summarize(flwr = sum(flwr)/n()) %>% 
  summarize(flwr_mean = mean(flwr),
            flwr_sd = sd(flwr))
```
