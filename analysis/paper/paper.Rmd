...
Precipitation and evapotranspiration data were obtained from a gridded dataset (0.25º x 0.25º resolution) built using data from 3,625 ground-based weather stations across Brazil [@xavierDailyGriddedMeteorological2016]. In addition to precipitation records, this gridded dataset had estimates of potential evapotranspiration calculated using the Pennman-Monteith method. Monthly precipitation and potential evapotranspiration was used to calculate the standardized precipitation evapotranspiration index (SPEI) [@vicente-serranoMultiscalarDroughtIndex2010]. SPEI is a proxy for meteorological drought that integrates precipitation and evapotranspiration anomalies over a specified time scale. More negative values of SPEI indicate greater drought intensity and positive values indicate wetter than average months. SPEI between 0 and -1 is considered mild drought, between -1 and -1.5 is considered moderate drought, between -1.5 and -2 is severe drought, and less than -2 is considered extreme drought [@mckee1993]. We chose a 3 month time scale as most relevant for *H. acuminata* as plants do not access sources of water buffered over longer time scales such as ground water or flood water <!--# work on this sentence -->. We calculated 3-month SPEI using the `SPEI` package in `r version$version.string` [@rcoreteamLanguageEnvironmentStatistical2020; @begueriaSPEICalculationStandardised2017]. We compared the SPEI values calculated using this data source to several others including near by weather stations, the global SPEI database <!--# citation --> and TRMM <!--# citation --> and the timing of drought events is consistent among these datasets although the exact magnitude of drought differs among datasets (Appendix 1)<!--# ref to appendix 1 -->.
...
To model growth, we used shoot number as a proxy for plant size. Shoot number is easy to measure accurately and, unlike plant height, does not change if a plant is knocked over by a treefall. <!--# I could consider an integrative measure like log(shts * ht) -->. Size in year $t+1$ was modeled as a function of the natural log of size in year $t$ and drought history (i.e. the crossbasis function) with plot as a random effect. We used a scaled t error family distribution with a log link since residuals were leptokurtic when a gaussian distribution was used.

For survival, plants were assumed dead the year after they were last observed if the plant was not observed for three or more years. Survival and flowering probability were similarly modeled as a function of size and drought history (i.e. the crossbasis function) with a random effect of plot, but with a binomial family error distribution and a logit link function.

Statistical significance of crossbasis functions and covariates was determined by ....

To better understand the shape of the crossbasis function, we calculated fitted values for an average sized plant given a range of possible SPEI values from \_\_\_ to \_\_\_ at different lag times from \_\_\_ to \_\_\_ months, holding all other SPEI values at the average across the dataset (\_\_\_). These results could then be plotted as heatmaps with the *ggplot2* package ( <!--# cite ggplot2 --> ).

One limitation of the DLNM approach is that it does not currently allow for modeling of interactions with a crossbasis function. This limitation meant that we were unable to model a potential size by environment (i.e. drought history) interaction, which may occur with many plant species [@tredennickSizebyenvironmentInteractionsNeglected2018]. To check for a size by environment interaction, we followed the methods of Tredennick et al. [-@tredennickSizebyenvironmentInteractionsNeglected2018] and fit a pair of generalized linear mixed effects models (GLMMs) for each vital rate using the *lme4* package <!--# cite lme4 -->. Rather than explicitly modeling the effects of drought history, we modeled environmental stochasticity with random effects of year. For each vital rate, we included an additional random effect of year on either the intercept or both the intercept and slope of the relationship between plant size and the response. We assessed the significance of the year-by-size interaction by testing the likelihood ratio of models with and without the interaction against an expected likelihood ratio distribution under the null hypothesis of no interaction [@tredennickSizebyenvironmentInteractionsNeglected2018].
