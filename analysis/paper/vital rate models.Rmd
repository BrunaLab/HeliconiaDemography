...
```{r}
sr <- range(demog_post_cf$log_size)

newdata_covar_cf <- 
  tibble(log_size = seq(sr[1], sr[2], length.out = 100),
         plot = "newplot",
         year_fac = "newyear", 
...
```
```{r}
sr <- range(demog_post_1ha$log_size)
newdata_covar_1ha <- 
  tibble(log_size = seq(sr[1], sr[2], length.out = 100),
         plot = "newplot",
         year_fac = "newyear",
...
mean_size <- exp(ref_data$log_size)
```
...
covar_s_cf <-
  ggplot(out, aes(x = exp(log_size), y = plogis(.fitted))) + 
...
Uh...There might be something wrong with how I've determined survival.  I should revisit this.
