...
There are plants with 0 shoots and 0 ht (not sure what that means yet), so to log-transform, I need to first add a small number

```{r}
demog_full <-
  demog_full %>% 
  mutate(log_shts = log(shts + 0.0001), .after = shts) %>% 
  mutate(log_shts_next = log(shts_next + 0.0001), .after = shts_next) %>% 
  mutate(size = shts*ht, size_next = shts_next*ht_next)
```
...
g_post_cf <-
  gam(shts_next ~ 
      ...
      # family = scat(link = "log"), #like leptokurtic gaussian.
      data = demog_post_cf,
