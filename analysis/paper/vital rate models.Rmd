...
```{r}
dates <- range(demog_post$year) %>% paste0(., "-01-01") %>% ymd()

spei <- ggplot(xa, aes(x = yearmonth, y = spei)) + 
  geom_line(aes(linetype = latlon)) +
  scale_x_yearmonth(limits = dates)
...
survival <-
  ...
  summarize(surv = sum(surv, na.rm = TRUE)/n()) %>% 
  ggplot(aes(x = year, y = surv, color = habitat)) + geom_line()
survival / spei
```
Looks like survival went down year after 2003 drought and durring 2007 drought.
...
```{r}
size <- demog_post %>% 
  ggplot(aes(x = year, y = size, color = habitat)) +
...
```
```{r}
demog_post_cf %>% filter(size_prev > 1500, surv == 0)

demog %>% filter(ha_id_number == 5310) %>% View()
```
...
```{r}
# draw(g_post_1ha)
draw(g_post_1ha, select = 3, continuous_fill = scale_fill_viridis_c(option = "inferno"), contour = FALSE, fun = "exp")
ggsave("function_smooth-cb.png")
```
