...
I'm going to start fitting these with `bam()` to speed things up.
This sets up the paralellization.
CAUTION: you may need to change these settings for your machine.
...
`surv` is 1 in `year`s when a plant is alive and 0 in the first `year` when the plant is dead.
These GAMs model the survival of a plant (over the past year) as a function of size in the previous census and SPEI values starting at february of the current year going back 36 months.
\#\# Fit GAMs
...
Emilio suggested plotting curves from all 1-ha plots just to visualize the variation.
But the random effect of plot is not significant, so there is actually very little variation among plots in the model.
...
Ok, I want to add a lil strip on the bottom that shows dry and wet seasons.
...
AIC for... Gaussian: \~40909.27.
looks leptokurtic scaled t: \~37284.47.
Waaay better
