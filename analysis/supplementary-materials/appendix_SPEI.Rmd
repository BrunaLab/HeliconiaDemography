...
-   Daily gridded meteorological variables in Brazil [@xavierDailyGriddedMeteorological2016]
    ...
    -   PET calculated by ... method

    -   Source: <!--# number --> ground-based weather stations across Brazil

    -   Date range: 1980--2015

-   Daily precipitation grids for South America [@liebmannDailyPrecipitationGrids2005] <!--# Not sure this one even makes sense to use, as it doesn't include ETO or temperature data, so I'd have to get that from Xavier or another source. -->

    -   Resolution 1º x 1º

    -   Includes only precipitation

    -   PET calculated by \_\_\_ method using \_\_\_ data

    -   Source: <!--# number --> ground-based weather stations across South America

    -   Date range: 1987--2012

-   Global SPEI [@santiago2017]

    -   Resolution: 0.5º x 0.5º

    -   Provides SPEI calculated using monthly precipitation and potential evapotranspiration estimated with the FAO-56 Pennman-Monteith method, which is considered a superior method to Thornwaite, which only takes temperature into account.

    -   Source: CRU TS version 4.03, a gridded product based on land stations.
...
```{r sa-spei}
# sa # I'm not sure this one is worth adding to the comparison since there is no temperature or ETO data associated with it.
...
# For Cronians talk
Just gonna make a simple-ish plot (lol) with major droughts labeled
...
xa_df <-
  ...
  summarize(`Xavier et al.` = mean(xa_spei))

comb <- left_join(xa_df, gspei %>% 
  mutate(yearmonth = yearmonth(date), date = NULL)) %>% 
  rename(`Global SPEI` = g_spei) %>% 
...
```{r}
p <-
  ggplot(comb %>% filter(dataset == "Xavier et al."), aes(x = yearmonth, y = spei, color = dataset)) +
  geom_line() +
  scale_color_manual(values = c("#3b1255", "darkgreen")) +
  labs(y = "SPEI")
p <- annotate_spei(p)

p <-
  p +
  scale_x_yearmonth(date_breaks = "3 years",
                    date_labels = "%Y") +
  theme_bw() +
...
p
ggsave(here("spei_xavier.png"), p, width = 10, height = 4, units = "in")
```


Try labeling droughts

```{r}
library(gghighlight)
p + gghighlight(year(yearmonth) ==2010, use_direct_label = FALSE)
```
