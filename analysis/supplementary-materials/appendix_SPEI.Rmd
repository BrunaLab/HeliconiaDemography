...
labs <- comb %>% 
  ...
  mutate(xend = mean(yearmonth))

p <-
  ggplot(comb, aes(x = yearmonth, y = spei)) +
  geom_line(aes(linetype = dataset), size = 0.5) +
  # geom_label_repel(data = comb %>% filter(!is.na(drought_label)),
  #                  aes(label = drought_label, y = -2.5))
  geom_segment(data = labs,
               aes(xend = yearmonth, yend = spei -1)) +
  geom_label(data = labs,
             aes(x = yearmonth, y = spei-1, label = year, fill = drought_label)) +
  scale_x_yearmonth(date_breaks = "2 years") +
  scale_color_brewer(type = "qual", palette = 2) +
  coord_cartesian(xlim = c(yearmonth(ymd("1997-01-01")), yearmonth(ymd("2010-11-01")))) +
  theme_bw()
annotate_spei(p)
```
