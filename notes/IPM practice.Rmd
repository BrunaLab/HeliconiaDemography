...
unique(demog$ranch)
unique(demog$code_notes)
unique(demog$code2)
unique(demog$habitat)
...
- `plot` (character):
- `habitat` (character): fragment size
- `ranch` (character): "DIM" = Dimona, "EST" = Esteio-Colosso, or "PAL" = PortoAlegre.  The three ranches that make up BDFFP.
- `bdffp_reserve_no` (character): Official BDFFP reserve designation
- `ha_id_number` (numeric): A unique ID number for each plant after resolving duplicate tags and such.
...
- `code.notes` (character): 
    ...
    - "tag missing" = ID tag labeling the plant was missing.  Plants were given new tags and researchers did their best to match up new tag and old tag.
...
```{r}
km41 %>% group_by(ha_id_number) %>% summarize(max(year))
max(km41$year)
```
...
In this particular dataset the only information on reproduction is number of inflorescences per plant and total number of seedlings in a year.  However, Bruna et al. 2004 has some additional data on reproduction:

- each inflorescence has 20--25 flowers
- proportion of flowers developing into fruits = 0.30±0.24 (range = 0--0.88)
- Number of seeds per plant = 4.74±6.10 (range = 0--27)
- Number of sees per fruit =  1, 2, or 3 with equal probability.
- Number of seeds per flower = 0.27±0.33 (range = 0--1)

But the last step, seedlings per seed, isn't there, so I'm not sure if this info is relevant for this IPM, especially if none of the above vital rates is expected to differ by plant size.
...
```{r}
meo_pflwr2 <- glmer(flowered ~ ht + I(ht^2) + (1|ha_id_number) + (1|year), family = binomial, data = km41_2)
```
Check random effects

```{r}
mo_pflwr2 <- glmer(flowered ~ ht + I(ht^2) + (1|ha_id_number), family = binomial, data = km41_2)
...
```{r}
meo_pflwr <- glmer(flowered ~ ht + (1|ha_id_number) + (1|year), family = binomial, data = km41_2)
...
```{r}
meo_flwrs2 <- glmer(infl ~ ht + I(ht^2) + (1|ha_id_number) + (1|year), family = poisson, data = km41_flowered)
```
```{r}
me_flwrs2 <- glmer(infl ~ ht + I(ht^2) + (1|year), family = poisson, data = km41_flowered)
mo_flwrs2 <- glmer(infl ~ ht + I(ht^2) + (1|ha_id_number), family = poisson, data = km41_flowered)
...
km41_rec_sum <- 
  km41 %>%
  group_by(year) %>% 
  summarize(recruits = sum(code_notes == "sdlg (1)", na.rm = TRUE),
...
```{r}
km41_rec <- km41 %>% filter(code_notes == "sdlg (1)")
...
```{r}
meo_grow2 <- lmer(ht ~ ht_prev + I(ht_prev^2) + (1|ha_id_number) + (1|year), data = km41_2)
```
Test random effects

```{r}
me_grow2 <- lmer(ht ~ ht_prev + I(ht_prev^2) + (1|year), data = km41_2)
mo_grow2 <- lmer(ht ~ ht_prev + I(ht_prev^2) + (1|ha_id_number), data = km41_2)
...
### Model
```{r}
meo_surv2 <- glmer(surv ~ ht + I(ht^2) + (1|ha_id_number) + (1|year), family = binomial, data = km41_2)
```
```{r}
me_surv2 <- glmer(surv ~ ht + I(ht^2) + (1|year), family = binomial, data = km41_2)
mo_surv2 <- glmer(surv ~ ht + I(ht^2) + (1|ha_id_number), family = binomial, data = km41_2)
...
matplot2(sens, xlab = "Height")
```

Can't remember how to interpret this

```{r}
elas <- elasticity(out$K)
rownames(elas) <- colnames(elas) <- out$meshpts
matplot2(elas, xlab = "Height")
```
