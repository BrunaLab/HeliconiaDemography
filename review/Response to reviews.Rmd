...
Because model selection was performed automatically by shrinkage using the `select = TRUE` option in the `gam()` function, we cannot show relative model fits (i.e., we did not perform model selection and can't show dAIC).
However, we have now included a table with R^2^ values for each model indicating fit as well as estimated degrees of freedom and p-values for each term.
...
<!--# The following isn't in the main text yet.  Just ideas. -->

(Something I'm considering adding to the discussion):

"The main limitation for detecting lagged effects with any method is data, and the specifics of data limitations have not yet been systematically explored. @tellerLinkingDemographyDrivers2016 used simulations to show that 20--25 years of data were required to detect lagged effects using their method, but this cutoff depends on the effect size and the number of climate extremes in the dataset. Additionally, there is some evidence that appropriate spatial replication could drastically reduce the number of years of data required (Aldo Compagnoni's ESA talk)."
...
We have edited the text to omit "growth", and hopefully clarify: "...the impact of plant size on size in the following year was greatest for mid-sized plants in continuous forest." <!--# are there other edits we can make to clarify?  A sentence introducing the concept that size t+1 ~ size t *is* growth? -->
...
We are a little hesitant to speculate too much here, as not much is known about the optimal microclimate conditions for flowering in *H. acuminata*, but we have revised this paragraph (see also our response to @ref(R2-17). It now includes the following possible explanation: *"Drought conditions could be favorable for H. accuminata flowering due to increased temperatures or decreased cloudiness associated with droughts[@pauCloudsTemperatureDrive2013],* *and the effects could be delayed due to the development time of inflorescences."*
...
We have reverted the figure to this version (see response below) and this should be obvious now.
...
We have not clarified that in the figure caption.
...
The reference, @brunaHabitatFragmentationPlant2009, discusses the lack of knowledge of what demographic vital rates (e.g. survival, reproduction, recruitment) in which life stages (e.g. small or large individuals) contribute most to population declines in fragments.
...
...see also our response to \@ref(R1-5).
<!--# R1-5 is asking for more discussion on limitations of DLNMs -->
...
...see also our response to \@ref(R1-5) and \@ref(R2-13).
...
We believe that the bet-hedging argument was confusing for two reasons: 1) it assumed that drought (as we measured it with SPEI) is detrimental to *H. accuminata*, which is not consistently reflected in our results, and 2) it did not address longer lags than the development time of a flowering shoot.
...
<!--# Emilio: I feel like I don't have a great response to this one.  Is there some edit to the manuscript that would clarify? I don't think "let the model do the smoothing" accurately explains what would happen if we switched to SPEI_1 -->

\
\
The choice of scale for the SPEI calculation is indeed difficult, and contributes to the definition of drought (i.e. is drought defined as dry conditions over the previous 1, 2, 3 or more months?)
. A thorough comparison of the ability for different SPEI scales to explain variation in survival, size, and flowering is beyond the scope of this paper
.
...
<!--# yes, but might change number of knots, so might require more work revising this -->
