...
# TODO:
- Fix x,y coords correctly.  Move this code to demography_wrangling.R?
...
```{r}
#density between 0m and 10m away
dist_0_10 <-
  dist_full %>% 
  filter(dist >= 0, dist < 10) %>% 
  group_by(row_from, column_from) %>% 
  summarize(n_neighbors_0_10 = sum(n_plants, na.rm = TRUE),
            n_cells = n(),
            density_0_10 = n_neighbors_0_10/n_cells,
            shtsper_0_10 = sum(n_shts, na.rm = TRUE)/n())
```
...
```{r}
full_test <- full_join(ha_test, dist_0_10, by = c("row_number" = "row_from", "column" = "column_from"))
head(full_test)
```
For the nearest distances (0-10m), I need to subtract the number of shoots in the focal plant.

```{r}
full_test %>% 
  # mutate(shtsper_0_10 = shtsper_0_10 - shts) %>% 
  arrange(row, column)

full_test %>% 
  group_by(row, column) %>% 
  summarize(n = n(), n_shts = sum(shts))
```
...
ha_new
```
Finally, I need to correct the closest density by subtracting shoots of the focal plant.

```{r}
ha_new <- ha_new %>% mutate(density_0_10 = density_0_10 - shts)
```
...
# Using exact x, y coords
```{r}
library(furrr)
plan(multisession)
```

For a subset of plants in 2009, exact x,y coords in the plot were taken.  Use these to estimate density.

```{r}
ha_xy <-
  ha_new %>% 
  filter(!is.na(x_09) & !is.na(y_09))
ha_xy
```
It looks like the x, y coords are *within* each grid cell (e.g. A1).  Need to offset them to get relative coords for entire plot.  I'll make A1 = 0,0

```{r}
ha_xy <-
  ha_xy %>% 
  mutate(x_adj = x_09 + ((column - 1) * 10),
         y_adj = y_09 + ((row_number - 1) * 10),
         .after = y_09)
```
...
ha_xy_s <-
  ...
  summarize(x = mean(x_adj),
            y = mean(y_adj))
#down to 3,571--still too big to do dist(), I think?

test <- ha_xy_s %>% select(x, y) %>% as.matrix()
rownames(test) <- paste(test[,1], test[,2], sep = ",")
head(test)
dist(test[1:5,])
...
For every ha_id_number, find only plants within 0-10 m, sum the number of shoots.
...
ha_xy %>% 
  ...
  rowwise() %>% 
  filter(eu_dist(
    c(first(cur_data()$x_adj), first(cur_data()$y_adj)),
    c(x_adj, y_adj)) < 10)
```
...
add_density_cols <- function(df, L, U, by, verbose = FALSE) {
  ...
  df_out <- 
    future_map_dfr(1:nrow(df), function(r) { #loop through rows/plants
      ...
      focal_plant <- t(df[r, c("x_adj", "y_adj")])
      
      #calculate distance to every other plant
      focal_df <-
        df %>%
        rowwise() %>%
        mutate(dist = eu_dist(focal_plant, c(x_adj, y_adj)))
...
Uggggh only works for first plot???