...
# assessing seed vs. safe-site limitation. Ecology 91:1317–1328.
...
# U = 10
# by = 0.5
add_density_cols <- function(df, L, U, by, na_edges = TRUE) {
...
ha_xy2 <-
  ...
  future_map(~{
      add_density_cols(.x, L = 0, U = 10, by = 0.5) 
    }) %>% 
  bind_rows()
beepr::beep(4)
```

Aggregate some of the distances for simpler modeling.

```{r}
ha_xy3 <-
  ha_xy2 %>% 
  rowwise() %>%
  mutate(shts_0_5 = sum(c_across(shts_0_0.5:shts_4.5_5)),
         shts_5_10 = sum(c_across(shts_5_5.5:shts_9.5_10)))
