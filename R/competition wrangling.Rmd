...
```
So at [1,1] there are only 3 cells between 5 and 15m away, there are a total of 6 plants in those cells, so the conspecific density at that distance is 2.0 plants / 100m^2
...
Now, create that summary tibble counting the number of plants in each grid cell for every plot:year combo, join, and unnest.

```{r}
ha_dist <-
  ha_nest_dist %>% 
  #summarize each year:plot to count number of plants in each grid cell
  mutate(n_plants_sum = map(data, ~{
    .x %>% 
      add_column(plant = 1) %>% 
      group_by(row_number, column) %>% 
      summarize(n_plants = n()) %>% 
  ...
  mutate(dist_full = map2(dist_df, n_plants_sum, ~{
...
dens_0_10 <-
  ...
  summarize(density_0_10 = sum(n_plants, na.rm = TRUE)/n())
...
out <- 
  ...
  ha_dist %>%
    ...
    group_by(plot, year, row_from, column_from) %>%
    summarize(!!glue::glue("density_{.x}_{.y}") := sum(n_plants, na.rm = TRUE)/n())
})
