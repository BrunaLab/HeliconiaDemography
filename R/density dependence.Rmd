...
```{r}
ggplot(pred_growth %>% filter(min_dist < 0.1),
...
```{r}
ggplot(pred_growth %>% filter(lag == 0.75),
       aes(x = x, y = exp(fitted), ymin = exp(fitted - 1.96*se.fit), ymax = exp(fitted + 1.96*se.fit))) +
  geom_line() + 
  geom_ribbon(alpha = 0.3) +
  geom_point(data = ha_sub, aes(x = shts_0.5_1, y = shts_next), inherit.aes = FALSE) +
  coord_cartesian(ylim = c(0,20))
```


- TODO: Try with Florestal (really huge plants there)
