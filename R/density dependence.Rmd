...
```{r}
mnorm4 <- glmer(shts ~ log(shts_prev) * density_0_10 + (1|year_post),
               family = gaussian(link = "log"),
               data = ha_cf)
AICtab(mnorm3, mnorm4)
```

Keep plant ID as a random effect.

```{r}
mnorm5 <- glmer(shts ~ log(shts_prev) * density_0_10 + (1|ha_id_number),
               family = gaussian(link = "log"),
               data = ha_cf)
AICtab(mnorm3, mnorm4, mnorm5)
```
