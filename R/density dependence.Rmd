...
```{r}
m_surv <- glmer(surv ~ log_shts_prev + density_0_10 + density_10_20 + (1|plot) + (1|ha_id_number) + (1|year), 
...
```{r}
m_surv2 <- glmer(surv ~ log_shts_prev + density_0_10 + density_10_20 + (1|plot) + (1|year), 
                family = binomial, data = ha_cf)
AICtab(m_surv, m_surv2)
```
Can remove plant ID
```{r}
m_surv3 <- glmer(surv ~ log_shts_prev + density_0_10 + density_10_20 + (1|year), 
                family = binomial, data = ha_cf)
...
```{r}
m_surv4 <- glmer(surv ~ log_shts_prev + density_0_10 + density_10_20 + (1|plot), 
                family = binomial, data = ha_cf)
...
Yes, but only immediate grid cell
