...
```{r}
m_growth2 <- glmer(shts_next ~ log_shts + habitat + shts_0_5 + shts_5_10 + (1|year_post),
               family = gaussian(link = "log"),
               data = ha_sub)
...
```{r}
m_growth3 <- glmer(shts_next ~ log_shts + habitat + shts_0_5 + shts_5_10 + (1|ha_id_number),
               family = gaussian(link = "log"),
               data = ha_sub)
...
```{r}
mgrowth_sub <- glmer(shts_next ~ log_shts + habitat + shts_0_5 + shts_5_10 + (1|ha_id_number) + (1|year_post),
                family = gaussian(link = "log"),
                data = ha_sub_sub)
Anova(mgrowth_sub)
```
...
```{r}
m_surv <- glmer(surv ~ log_shts + habitat + shts_0_5 + shts_5_10 + (1|ha_id_number) + (1|year), 
...
```{r}
m_surv2 <- glmer(surv ~ log_shts + habitat + shts_0_5 + shts_5_10 + (1|year), 
                family = binomial, data = ha_sub)
...
```{r}
m_surv2_sub <- glmer(surv ~ log_shts + habitat + shts_0_5 + shts_5_10 + (1|year), 
                family = binomial, data = ha_sub_sub)
Anova(m_surv2_sub)
```
...
```{r}
m_flwr <- glmer(flwr ~ log_shts + habitat + shts_0_5 + shts_5_10 + (1|ha_id_number) + (1|year),
                family = binomial, data = ha_sub)
```
...
```{r}
m_flwr2 <- glmer(flwr ~ log_shts + habitat + shts_0_5 + shts_5_10 + (1|year),
                 family = binomial, data = ha_sub)
...
```{r}
m_flwr3 <- glmer(flwr ~ log_shts + habitat + shts_0_5 + shts_5_10 + (1|ha_id_number),
                 family = binomial, data = ha_sub)
...
```{r eval=FALSE}
# x <- crosspred("Q", model = g_surv, cumul = TRUE)
...
plot.crosspred(x,
               ...
               cumul = FALSE)
```
So this shows negative density dependence on growth that seems to be strongest at about 40-50 m away from the focal plant.
